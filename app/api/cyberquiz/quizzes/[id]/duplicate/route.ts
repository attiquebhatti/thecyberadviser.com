...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    const [[src]] = await pool.query('SELECT * FROM quizzes WHERE id = ? AND host_id = ?', [params.id, auth.user.id]) as any[];
    if (!src) return NextResponse.json({ error: 'Quiz not found' }, { status: 404 });

    const newId = uuidv4();
    await pool.query('INSERT INTO quizzes (id, host_id, title, subject, grade_level, is_public) VALUES (?, ?, ?, ?, ?, ?)',
      [newId, auth.user.id, `${src.title} (copy)`, src.subject, src.grade_level, 0]);

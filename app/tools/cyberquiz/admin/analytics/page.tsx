...
export default function AnalyticsPage() {
  ...
  if (loading || !user || allowed === null) {
    return <div className="min-h-screen bg-[#0f0f1a] flex items-center justify-center"><Loader2 className="w-6 h-6 text-[#6bd348] animate-spin" /></div>;
  }
  if (!allowed) {
    return (
      <div className="min-h-screen bg-[#0f0f1a] flex items-center justify-center px-4">
        <div className="text-center">
          ...
          <button onClick={() => router.push(`${BASE}/dashboard`)} className="text-sm text-[#6bd348] hover:underline">Back to dashboard</button>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#0f0f1a]">
      <header className="bg-[#1a1a2e] border-b border-[#2d2d44] px-4 py-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-[#f1f5f9] flex items-center gap-2"><TrendingUp className="w-5 h-5 text-[#6bd348]" /> Website Traffic</h1>
          ...
          {configured && (
            <select value={days} onChange={(e) => { setInsights(null); setDays(parseInt(e.target.value, 10)); }}
              className="rounded-lg bg-[#0f0f1a] border border-[#2d2d44] px-3 py-2 text-sm text-[#f1f5f9] focus:outline-none focus:border-[#6bd348]/60">
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 space-y-6">
        {!configured ? (
          ...
          <div className="flex items-center justify-center py-16"><Loader2 className="w-6 h-6 text-[#6bd348] animate-spin" /></div>
        ) : (
          <>
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
              ...
              <Stat icon={Users} label="Active Users" value={insights.totals.activeUsers} color="#6bd348" />
            ...
            <div className="rounded-xl border border-[#2d2d44] bg-[#1a1a2e] p-5">
              <p className="text-sm font-semibold text-[#f1f5f9] mb-4 flex items-center gap-2"><Activity className="w-4 h-4 text-[#6bd348]" /> Daily Active Users</p>
              <div className="flex items-end gap-1 h-40">
                {insights.trend.map((t) => (
                  <div key={t.date} className="flex-1 group relative flex flex-col justify-end items-center">
                    <div className="w-full rounded-t bg-gradient-to-t from-[#6bd348] to-[#a855f7] transition-all hover:opacity-80"
...
function Panel({ icon: Icon, title, children }: { icon: any; title: string; children: React.ReactNode }) {
  return (
    <div className="rounded-xl border border-[#2d2d44] bg-[#1a1a2e] p-5">
      <p className="text-sm font-semibold text-[#f1f5f9] mb-3 flex items-center gap-2"><Icon className="w-4 h-4 text-[#6bd348]" /> {title}</p>

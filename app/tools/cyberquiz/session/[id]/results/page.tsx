        ...
        <div className="flex gap-1 bg-[#1a1a2e] border border-[#2d2d44] rounded-xl p-1 w-fit">
          {(['leaderboard', 'questions', 'heatmap'] as const).map(t => (
            <button key={t} onClick={() => setActiveTab(t)}
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-all capitalize ${activeTab === t ? 'bg-[#6bd348] text-[#04130c]' : 'text-[#94a3b8] hover:text-[#f1f5f9]'}`}>
              {t}
            </button>
          ))}
        </div>
              ...
              <tbody>
                {leaderboard.map(p => (
                  <tr key={p.id} className="border-b border-[#2d2d44] last:border-0 hover:bg-white/2 transition-colors">
                    ...
                    <td className="px-4 py-3 text-right font-bold text-[#6bd348]">{formatScore(p.score)}</td>
            ...
            <div className="rounded-xl bg-[#1a1a2e] border border-[#2d2d44] p-5">
              ...
              <ResponsiveContainer width="100%" height={200}>
                <BarChart data={accuracyData}>
                  ...
                  <Bar dataKey="accuracy" fill="#6bd348" radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
                      ...
                      return (
                        <div key={ci} className="flex-1 text-center">
                          <div className="h-16 relative rounded-lg overflow-hidden bg-[#0f0f1a]">
                            ...
                            <span className={`absolute top-1 left-1/2 -translate-x-1/2 text-xs font-bold ${isCorrect ? 'text-[#6bd348]' : 'text-[#f87171]'}`}>
                              {['A', 'B', 'C', 'D'][ci]}
                            </span>
                          </div>
                ...
                <tbody>
                  {leaderboard.map(p => (
                    <tr key={p.id} className="border-t border-[#2d2d44]">
                      ...
                      <td className="py-2 pl-4 text-right font-bold text-[#6bd348]">{formatScore(p.score)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>

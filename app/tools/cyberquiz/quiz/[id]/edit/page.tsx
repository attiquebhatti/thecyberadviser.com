...
function SortableQuestion({ question, index, selected, onClick, onDelete }: {
  ...
  return (
    <div ref={setNodeRef} style={style}
      className={`flex items-center gap-2 p-3 rounded-lg cursor-pointer transition-all group ${selected ? 'bg-[#6bd348]/20 border border-[#6bd348]/50' : 'bg-[#0f0f1a] border border-[#2d2d44] hover:border-[#3d3d5a]'}`}
      onClick={onClick}>
      ...
      <header className="sticky top-24 z-30 bg-[#1a1a2e] border-b border-[#2d2d44] px-4 py-3">
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <input type="text" value={title} onChange={e => setTitle(e.target.value)}
            className="flex-1 min-w-0 bg-transparent text-lg font-bold text-[#f1f5f9] focus:outline-none placeholder:text-[#4a4a6a] border-b-2 border-transparent focus:border-[#6bd348] transition-colors"
            placeholder="Quiz title…" />
          <select value={subject} onChange={e => setSubject(e.target.value)}
            className="bg-[#0f0f1a] border border-[#2d2d44] text-[#94a3b8] text-sm rounded-lg px-2 py-1.5 focus:outline-none focus:border-[#6bd348]">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className="w-64 shrink-0 bg-[#1a1a2e] border-r border-[#2d2d44] flex flex-col">
          ...
          <div className="p-3 border-t border-[#2d2d44]">
            <button onClick={addQ} className="w-full flex items-center justify-center gap-2 py-2.5 rounded-lg border-2 border-dashed border-[#2d2d44] hover:border-[#6bd348]/50 text-[#94a3b8] hover:text-[#f1f5f9] text-sm transition-all">
            ...
            <motion.div key={selectedIdx} initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} className="max-w-2xl mx-auto space-y-5">
              ...
              <div className="flex flex-wrap gap-3">
                <select value={currentQ.type} onChange={e => updateQ('type', e.target.value)}
                  className="bg-[#1a1a2e] border border-[#2d2d44] text-[#94a3b8] text-sm rounded-lg px-3 py-2 focus:outline-none focus:border-[#6bd348]">
              ...
              {currentQ.type === 'multiple_choice' && (
                <div className="space-y-2">
                  ...
                  {(currentQ.options as QuestionOption[]).length < 6 && (
                    <button onClick={addOpt} className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl border-2 border-dashed border-[#2d2d44] hover:border-[#6bd348]/50 text-[#94a3b8] hover:text-[#f1f5f9] text-sm transition-all">
                  ...
                  <div className="flex gap-3">
                    {['True', 'False'].map(v => (
                      <button key={v} onClick={() => updateQ('correct_answer', v)}
                        className={`flex-1 py-3 rounded-xl border-2 font-semibold transition-all ${currentQ.correct_answer === v ? 'border-[#22c55e] bg-[#22c55e]/10 text-[#6bd348]' : 'border-[#2d2d44] text-[#94a3b8] hover:border-[#3d3d5a]'}`}>
                        {v}
                      </button>
                    ))}
                  </div>
              ...
              {(currentQ.type === 'short_answer' || currentQ.type === 'fill_blank') && (
                <div>
                  ...
                  <input type="text" value={currentQ.correct_answer || ''} onChange={e => updateQ('correct_answer', e.target.value)}
                    ...
                    className="w-full px-4 py-2.5 rounded-xl bg-[#1a1a2e] border border-[#2d2d44] text-[#f1f5f9] placeholder:text-[#4a4a6a] focus:outline-none focus:border-[#6bd348] transition-colors" />
                </div>
              )}
              ...
              <div>
                ...
                <textarea value={currentQ.explanation || ''} onChange={e => updateQ('explanation', e.target.value)} rows={2}
                  ...
                  className="w-full px-4 py-2.5 rounded-xl bg-[#1a1a2e] border border-[#2d2d44] text-[#f1f5f9] placeholder:text-[#4a4a6a] focus:outline-none focus:border-[#6bd348] transition-colors resize-none text-sm" />
              </div>
            </motion.div>
          ) : (

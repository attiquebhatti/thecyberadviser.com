...
const GAME_MODES = [
  { id: 'classic_blitz',  name: 'Classic Blitz',    icon: Zap,      color: '#7c3aed', bg: 'from-[#7c3aed]/20 to-[#7c3aed]/5', desc: 'Speed + accuracy = points. Perfect for any group.', free: true  },
  ...
  { id: 'infinity_mode',  name: 'Infinity Mode',    icon: Infinity, color: '#818cf8', bg: 'from-[#818cf8]/20 to-[#818cf8]/5', desc: 'Endless loop. Currency keeps accumulating.',          free: false },
];
          ...
          <div className="space-y-3 flex-1">
            ...
            {[
              ...
              <label key={key} className="flex items-center justify-between cursor-pointer">
                ...
                <button
                  className={`w-10 h-5 rounded-full transition-colors ${settings[key as keyof typeof settings] ? 'bg-[#7c3aed]' : 'bg-[#2d2d44]'} relative`}
            ...
            {GAME_MODES.map(mode => {
              ...
              return (
                <motion.button key={mode.id} whileHover={!locked ? { scale: 1.02 } : {}} whileTap={!locked ? { scale: 0.98 } : {}}
                  ...
                  {locked && <p className="text-xs text-[#7c3aed] mt-1">Upgrade to unlock</p>}
                </motion.button>
              );
            })}
          </div>
